...
# 1D compressible Euler equations: Sod shock tube
#
...
#
# Network output: primitive variables W=(rho,u,p), with rho>0 and p>0 enforced
...
#   - exact Sod solution is used only for evaluation/plotting
# ============================================================
...
class EulerSodConfig:
    ...
    # Sod shock-tube domain and states
    x_min: float = -0.5
    x_max: float = 0.5
    t_min: float = 0.0
    t_max: float = 0.2
    ...
    # Training budget. These are iterations with resampled collocation points.
...
def configure_runtime(new_cfg: EulerSodConfig):
...
def make_run_dir(cfg: EulerSodConfig) -> Path:
...
# ============================================================
# 3. Problem scales and schedule
# ============================================================

def scales(cfg: EulerSodConfig):
    rho_scale = max(cfg.rhoL, cfg.rhoR, 1.0)
    p_scale = max(cfg.pL, cfg.pR, 1.0)
    cL = math.sqrt(cfg.gamma * cfg.pL / cfg.rhoL)
    cR = math.sqrt(cfg.gamma * cfg.pR / cfg.rhoR)
    u_scale = max(abs(cfg.uL), abs(cfg.uR), cL, cR, 1.0)
...
def h0(cfg: EulerSodConfig):
    return (cfg.x_max - cfg.x_min) / math.sqrt(cfg.n_f)


def schedule_from_progress(progress: float, cfg: EulerSodConfig):
...
def schedule_from_iter(iteration: int, total_iters: int, cfg: EulerSodConfig):
    return schedule_from_progress(iteration / max(1, total_iters), cfg)


# ============================================================
# 4. Exact Sod solver for evaluation only
# ============================================================
...
def solve_star_region(cfg: EulerSodConfig):
    g = cfg.gamma
...
def sod_wave_speeds(cfg: EulerSodConfig):
    g = cfg.gamma
    gm1 = g - 1.0
    gp1 = g + 1.0
...
def euler_exact_np(X, T, cfg: EulerSodConfig):
    X = np.asarray(X)
    T = np.asarray(T)
...
def primitive_to_conserved_np(rho, u, p, cfg: EulerSodConfig):
    m = rho * u
    E = p / (cfg.gamma - 1.0) + 0.5 * rho * u**2
    return rho, m, E


def flux_np(rho, u, p, cfg: EulerSodConfig):
...
class MLP(nn.Module):
    def __init__(self, cfg: EulerSodConfig):
        super().__init__()
        self.cfg = cfg
        ...
        rho_mean = 0.5 * (cfg.rhoL + cfg.rhoR)
        p_mean = 0.5 * (cfg.pL + cfg.pR)
        with torch.no_grad():
            self.out.bias[0].fill_(inv_softplus(rho_mean - cfg.rho_floor))
            self.out.bias[1].fill_(0.5 * (cfg.uL + cfg.uR))
            self.out.bias[2].fill_(inv_softplus(p_mean - cfg.p_floor))
    ...
    def forward(self, xt):
        ...
        u = raw[:, 1:2]
        p = self.p_floor + F.softplus(raw[:, 2:3])
        return torch.cat([rho, u, p], dim=1)


def primitive_to_conserved_torch(W, cfg: EulerSodConfig):
    rho = W[:, 0:1]
    u = W[:, 1:2]
    p = W[:, 2:3]
    m = rho * u
    E = p / (cfg.gamma - 1.0) + 0.5 * rho * u.pow(2)
    return rho, m, E


def flux_torch(W, cfg: EulerSodConfig):
    rho, m, E = primitive_to_conserved_torch(W, cfg)
    u = W[:, 1:2]
    p = W[:, 2:3]
    return m, m * u + p, u * (E + p)


def euler_residual(model, x, t, cfg: EulerSodConfig):
    ...
    rho_t = torch.autograd.grad(rho, t, grad_outputs=torch.ones_like(rho), create_graph=True, retain_graph=True)[0]
    m_t = torch.autograd.grad(m, t, grad_outputs=torch.ones_like(m), create_graph=True, retain_graph=True)[0]
    E_t = torch.autograd.grad(E, t, grad_outputs=torch.ones_like(E), create_graph=True, retain_graph=True)[0]
    ...
    F3_x = torch.autograd.grad(F3, x, grad_outputs=torch.ones_like(F3), create_graph=True, retain_graph=True)[0]
    ...
    R2 = (r1 / rho_s).pow(2) + (r2 / mom_s).pow(2) + (r3 / e_s).pow(2)
    return W, r1, r2, r3, R2
...
def state_left(n: int, cfg: EulerSodConfig):
    rho = torch.full((n, 1), cfg.rhoL, device=DEVICE, dtype=DTYPE)
    u = torch.full((n, 1), cfg.uL, device=DEVICE, dtype=DTYPE)
    p = torch.full((n, 1), cfg.pL, device=DEVICE, dtype=DTYPE)
    return torch.cat([rho, u, p], dim=1)


def state_right(n: int, cfg: EulerSodConfig):
    rho = torch.full((n, 1), cfg.rhoR, device=DEVICE, dtype=DTYPE)
    u = torch.full((n, 1), cfg.uR, device=DEVICE, dtype=DTYPE)
    p = torch.full((n, 1), cfg.pR, device=DEVICE, dtype=DTYPE)
    return torch.cat([rho, u, p], dim=1)


def sample_ic(n: int, cfg: EulerSodConfig):
    nL = n // 2
    nR = n - nL
...
def sample_bc(n: int, cfg: EulerSodConfig):
    # Fixed reservoir states. Valid for the standard Sod t_max=0.2 on [-0.5,0.5]
    # because the waves do not reach the boundaries.
    nL = n // 2
    nR = n - nL
...
def sample_f(n: int, cfg: EulerSodConfig):
...
def scaled_primitive_mse(pred, target, cfg: EulerSodConfig):
...
def vanilla_loss(model, cfg: EulerSodConfig):
    ...
    _, _, _, _, R2 = euler_residual(model, x_f, t_f, cfg)

    loss_ic = scaled_primitive_mse(W_ic, y_ic, cfg)
    loss_bc = scaled_primitive_mse(W_bc, y_bc, cfg)
    loss_pde = R2.mean()
...
@torch.no_grad()
def trace_ratio_gate_centered(model, x, t, h_probe: float, cmin: float, cfg: EulerSodConfig):
    x_m1 = x - h_probe
    x_p1 = x + h_probe
    x_m2 = x - 2.0 * h_probe
    x_p2 = x + 2.0 * h_probe
    ...
    rho_s, u_s, p_s, _, _ = scales(cfg)
    d1_rho = (W_m1[:, 0:1] - W_p1[:, 0:1]) / rho_s
    d1_u = (W_m1[:, 1:2] - W_p1[:, 1:2]) / u_s
    d1_p = (W_m1[:, 2:3] - W_p1[:, 2:3]) / p_s
    d2_rho = (W_m2[:, 0:1] - W_p2[:, 0:1]) / rho_s
    d2_u = (W_m2[:, 1:2] - W_p2[:, 1:2]) / u_s
    d2_p = (W_m2[:, 2:3] - W_p2[:, 2:3]) / p_s

    J1 = torch.sqrt(d1_rho.pow(2) + d1_u.pow(2) + d1_p.pow(2) + 1.0e-12)
    J2 = torch.sqrt(d2_rho.pow(2) + d2_u.pow(2) + d2_p.pow(2) + 1.0e-12)
...
def trace_ratio_gated_loss(model, iteration: int, total_iters: int, cfg: EulerSodConfig):
    ...
    gate, C, Jhat, Jbar, valid = trace_ratio_gate_centered(model, x_f, t_f, h_probe, cmin, cfg)
...
def train_warmup_fixed(model, cfg: EulerSodConfig):
    model.train()
...
def train_vanilla_continuation_fixed(model, cfg: EulerSodConfig):
    model.train()
...
def train_gated_fixed(model, cfg: EulerSodConfig):
    model.train()
    opt = torch.optim.AdamW(model.parameters(), lr=cfg.lr_gated)
    sch = torch.optim.lr_scheduler.CosineAnnealingLR(opt, T_max=cfg.gated_iters, eta_min=cfg.lr_gated * 0.03)
    history = []
...
@torch.no_grad()
def predict_primitive_points(model, x_np, t_np, batch_size=65536):
    ...
    for s in range(0, x_flat.size, batch_size):
        ...
        out.append(to_numpy(model(xt)))
    W = np.concatenate(out, axis=0)
    return W.reshape(*np.asarray(x_np).shape, 3)


def predict_grid(model, cfg: EulerSodConfig, nx=None, nt=None):
    if nx is None:
        nx = cfg.eval_nx
    if nt is None:
        nt = cfg.eval_nt
    ...
    X, T = np.meshgrid(x, t)
    W = predict_primitive_points(model, X, T)
    rho, u, p = W[:, :, 0], W[:, :, 1], W[:, :, 2]
    rho_e, u_e, p_e = euler_exact_np(X, T, cfg)
    return x, t, X, T, rho, u, p, rho_e, u_e, p_e


def predict_slice(model, cfg: EulerSodConfig, t_value=None, nx=None):
    if t_value is None:
        t_value = cfg.t_plot
    if nx is None:
        nx = cfg.slice_nx
    x = np.linspace(cfg.x_min, cfg.x_max, nx)
    tt = np.full_like(x, t_value)
    W = predict_primitive_points(model, x, tt)
    return x, W[:, 0], W[:, 1], W[:, 2]


def compute_error_metrics(model, cfg: EulerSodConfig):
    x, t, X, T, rho, u, p, rho_e, u_e, p_e = predict_grid(model, cfg)
    out = {}
    for name, pred, exact in [("rho", rho, rho_e), ("u", u, u_e), ("p", p, p_e)]:
        err = pred - exact
        out[f"{name}_space_time_l1"] = float(np.mean(np.abs(err)))
        out[f"{name}_space_time_l2"] = float(np.sqrt(np.mean(err**2)))
        out[f"{name}_space_time_rel_l2"] = float(np.sqrt(np.mean(err**2)) / (np.sqrt(np.mean(exact**2)) + 1.0e-12))
        err_f = err[-1]
        exact_f = exact[-1]
        out[f"{name}_final_l1"] = float(np.mean(np.abs(err_f)))
        out[f"{name}_final_l2"] = float(np.sqrt(np.mean(err_f**2)))
        out[f"{name}_final_rel_l2"] = float(np.sqrt(np.mean(err_f**2)) / (np.sqrt(np.mean(exact_f**2)) + 1.0e-12))
        out[f"{name}_min"] = float(pred.min())
        out[f"{name}_max"] = float(pred.max())

    # Combined scaled primitive relative L2. This is useful for ranking median seeds.
    rho_s, u_s, p_s, _, _ = scales(cfg)
    scaled_err2 = ((rho - rho_e) / rho_s)**2 + ((u - u_e) / u_s)**2 + ((p - p_e) / p_s)**2
    scaled_exact2 = (rho_e / rho_s)**2 + (u_e / u_s)**2 + (p_e / p_s)**2
    out["primitive_scaled_space_time_rel_l2"] = float(np.sqrt(np.mean(scaled_err2)) / (np.sqrt(np.mean(scaled_exact2)) + 1.0e-12))

    rho_err_f = ((rho[-1] - rho_e[-1]) / rho_s)**2
    u_err_f = ((u[-1] - u_e[-1]) / u_s)**2
    p_err_f = ((p[-1] - p_e[-1]) / p_s)**2
    rho_ex_f = (rho_e[-1] / rho_s)**2
    u_ex_f = (u_e[-1] / u_s)**2
    p_ex_f = (p_e[-1] / p_s)**2
    out["primitive_scaled_final_rel_l2"] = float(np.sqrt(np.mean(rho_err_f + u_err_f + p_err_f)) / (np.sqrt(np.mean(rho_ex_f + u_ex_f + p_ex_f)) + 1.0e-12))

    # Positivity and TV diagnostics at final time.
    out["rho_positivity_violation"] = float(max(0.0, cfg.rho_floor - rho.min()))
    out["p_positivity_violation"] = float(max(0.0, cfg.p_floor - p.min()))
    for name, pred, exact in [("rho", rho, rho_e), ("u", u, u_e), ("p", p, p_e)]:
        tv_pred = np.sum(np.abs(np.diff(pred[-1])))
        tv_exact = np.sum(np.abs(np.diff(exact[-1])))
        out[f"{name}_tv_final"] = float(tv_pred)
        out[f"{name}_tv_excess_final"] = float(max(0.0, tv_pred - tv_exact))

    return out


def compute_global_conservation_metrics(model, cfg: EulerSodConfig):
    x, t, X, T, rho, u, p, _, _, _ = predict_grid(model, cfg)
    mass, mom, energy = primitive_to_conserved_np(rho, u, p, cfg)
    F1_L, F2_L, F3_L = flux_np(rho[:, 0], u[:, 0], p[:, 0], cfg)
    F1_R, F2_R, F3_R = flux_np(rho[:, -1], u[:, -1], p[:, -1], cfg)

    results = {}
    for name, U, FL, FR in [("mass", mass, F1_L, F1_R), ("mom", mom, F2_L, F2_R), ("energy", energy, F3_L, F3_R)]:
        integral = np.trapz(U, x, axis=1)
        flux_diff = FR - FL
        cons_vals = []
        rel_vals = []
        for i in range(len(t)):
            flux_int = np.trapz(flux_diff[: i + 1], t[: i + 1]) if i > 0 else 0.0
            val = integral[i] - integral[0] + flux_int
            denom = abs(integral[i] - integral[0]) + abs(flux_int) + 1.0e-12
            cons_vals.append(val)
            rel_vals.append(abs(val) / denom)
        cons_vals = np.asarray(cons_vals)
        rel_vals = np.asarray(rel_vals)
        results[f"global_{name}_cons_mean_abs"] = float(np.mean(np.abs(cons_vals)))
        results[f"global_{name}_cons_final_abs"] = float(abs(cons_vals[-1]))
        results[f"global_{name}_cons_mean_rel"] = float(np.mean(rel_vals[1:]))
    return results


def compute_local_conservation_metrics(model, cfg: EulerSodConfig):
    rng = np.random.default_rng(cfg.seed + 271828)
    vals = {"mass": [], "mom": [], "energy": []}
    rels = {"mass": [], "mom": [], "energy": []}
    for _ in range(cfg.n_control_volumes):
        ...
        W_t1 = predict_primitive_points(model, xq, np.full_like(xq, t1))
        W_t2 = predict_primitive_points(model, xq, np.full_like(xq, t2))
        rho1, u1, p1 = W_t1[:, 0], W_t1[:, 1], W_t1[:, 2]
        rho2, u2, p2 = W_t2[:, 0], W_t2[:, 1], W_t2[:, 2]
        U1 = primitive_to_conserved_np(rho1, u1, p1, cfg)
        U2 = primitive_to_conserved_np(rho2, u2, p2, cfg)

        W_a = predict_primitive_points(model, np.full_like(tq, a), tq)
        W_b = predict_primitive_points(model, np.full_like(tq, b), tq)
        Fa = flux_np(W_a[:, 0], W_a[:, 1], W_a[:, 2], cfg)
        Fb = flux_np(W_b[:, 0], W_b[:, 1], W_b[:, 2], cfg)

        for j, name in enumerate(["mass", "mom", "energy"]):
            int_t2 = np.trapz(U2[j], xq)
            int_t1 = np.trapz(U1[j], xq)
            flux_int = np.trapz(Fb[j] - Fa[j], tq)
            res = int_t2 - int_t1 + flux_int
            denom = abs(int_t2) + abs(int_t1) + abs(flux_int) + 1.0e-12
            vals[name].append(abs(res))
            rels[name].append(abs(res) / denom)

    out = {}
    for name in ["mass", "mom", "energy"]:
        v = np.asarray(vals[name])
        r = np.asarray(rels[name])
        out[f"local_{name}_cons_cv_mean_abs"] = float(v.mean())
        out[f"local_{name}_cons_cv_median_abs"] = float(np.median(v))
        out[f"local_{name}_cons_cv_max_abs"] = float(v.max())
        out[f"local_{name}_cons_cv_mean_rel"] = float(r.mean())
    return out


@torch.no_grad()
def compute_gate_diagnostics(model, cfg: EulerSodConfig, progress=1.0):
    ...
    gate, C, Jhat, Jbar, valid = trace_ratio_gate_centered(model, xt[:, 0:1], xt[:, 1:2], h_probe, cmin, cfg)
    ...
    return {
        ...
        "gate_active_frac_gt_0p5": float(((G > 0.5) & V).sum() / V.sum()),
        "gate_active_frac_gt_0p1": float(((G > 0.1) & V).sum() / V.sum()),
...
def evaluate_model(model, model_name: str, cfg: EulerSodConfig):
    row = {"model": model_name, "seed": cfg.seed}
    row.update(compute_error_metrics(model, cfg))
...
def print_header(cfg: EulerSodConfig, run_dir: Path):
    rho_s, u_s, p_s, mom_s, e_s = scales(cfg)
    h_start, c_start, _ = schedule_from_progress(0.0, cfg)
    h_end, c_end, _ = schedule_from_progress(1.0, cfg)
    speeds = sod_wave_speeds(cfg)
    x_left = cfg.x0 + speeds["left_head"] * cfg.t_max
    x_right = cfg.x0 + speeds["right_head"] * cfg.t_max
    print("=" * 90)
    print("1D Euler Sod shock tube: paired PINN vs trace-ratio gated PINN")
    print("=" * 90)
    print(f"seed                 : {cfg.seed}")
    print(f"device               : {DEVICE}")
    print(f"dtype                : {cfg.dtype}")
    print(f"domain               : x in [{cfg.x_min}, {cfg.x_max}], t in [{cfg.t_min}, {cfg.t_max}]")
    print(f"states               : L=(rho,u,p)=({cfg.rhoL},{cfg.uL},{cfg.pL}), R=({cfg.rhoR},{cfg.uR},{cfg.pR}), x0={cfg.x0}")
    print("PDE                  : conservative 1D Euler, U_t + F(U)_x = 0")
    print("network output       : primitive variables (rho,u,p), rho/p positivity via softplus floors")
    print(f"gamma                : {cfg.gamma}")
    print(f"Sod star state       : p*={speeds['p_star']:.6f}, u*={speeds['u_star']:.6f}")
    print(f"wave range at t_max  : left head x={x_left:.6f}, right wave x={x_right:.6f}")
    print("boundary condition   : fixed left/right reservoir primitive states")
    ...
    print("exact solution       : exact Sod solution used only for evaluation/plotting")
    print(f"run directory        : {run_dir}")
    print("=" * 90)


def run_one_seed_paired(seed: int, base_cfg: EulerSodConfig):
    ...
    if cfg_s.save_outputs:
        save_json({
            "config": asdict(cfg_s),
            "protocol": "shared warm-up; paired PINN continuation and trace-ratio gated continuation; same RNG restored; no validation; final checkpoint",
...
def summarize_multiseed(all_metrics: pd.DataFrame, save_dir: str):
    ...
    main_metrics = [
        "primitive_scaled_space_time_rel_l2",
        "primitive_scaled_final_rel_l2",
        "rho_space_time_rel_l2",
        "u_space_time_rel_l2",
        "p_space_time_rel_l2",
        "rho_final_rel_l2",
        "u_final_rel_l2",
        ...
        "u_tv_excess_final",
        "p_tv_excess_final",
        "rho_positivity_violation",
        "p_positivity_violation",
        "global_mass_cons_mean_abs",
        "global_mom_cons_mean_abs",
        "global_energy_cons_mean_abs",
        "local_mass_cons_cv_mean_abs",
        "local_mom_cons_cv_mean_abs",
        "local_energy_cons_cv_mean_abs",
    ]
    gate_metrics = [
    ...
    if len(tpinn_only) > 0 and len(gate_metrics) > 0:
...
def run_multiseed_euler_sod(base_cfg: EulerSodConfig, seeds):
    all_rows = []
    for seed in seeds:
...
base_cfg.device = "auto" 
base_cfg.save_outputs = True

base_cfg.width = 128
base_cfg.depth = 6

base_cfg.warmup_iters = 4000
base_cfg.gated_iters = 6000
base_cfg.n_f = 12000
base_cfg.n_ic = 1500
base_cfg.n_bc = 1500
base_cfg.w_ic = 100.0
...
# Uncomment the following block for a quick smoke test before full training.
# base_cfg.experiment_name = "euler_sod_trace_ratio_smoke_test"
...
# base_cfg.eval_nx = 80
# base_cfg.eval_nt = 40
# base_cfg.n_control_volumes = 4
# seeds = [2026]

RUN_LEGACY_EULER_TRAINING = False

if RUN_LEGACY_EULER_TRAINING:
    summary_results_euler = (
        run_multiseed_euler_sod(
            base_cfg,
            seeds,
        )
    )
...
def gpinn_residual_scaled_components(model, coordinates, cfg):
    ...
    W = model(cat_xt(x, t))
    rho, momentum, energy = primitive_to_conserved_torch(W, cfg)
    fluxes = flux_torch(W, cfg)
    conserved = (rho, momentum, energy)
    residuals = []
    for state, flux in zip(conserved, fluxes):
...
GPINN_HELPER_RECONSTRUCTION_SHA256 = '208f6226bfc67bda7efcf9269a0024ed1b150a3bd7118b7c53cc26ef7d7d0c48'
