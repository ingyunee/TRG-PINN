# ============================================================
# Paper-ready single-model Trace-Ratio Gated PINN
# 1D inviscid Burgers equation with Riemann shock data
#
# PDE: u_t + (u^2/2)_x = 0,  x in [x_min, x_max], t in [0, t_max]
# IC : u(x,0) = uL for x < x0, uR for x > x0, with uL > uR
#
# Protocol for paper main runs:
#   - single neural network
#   - vanilla PINN warm-up, then trace-ratio gated residual phase
#   - no validation set
#   - no early stopping
#   - no best-checkpoint selection
#   - final checkpoint evaluation only
#   - exact entropy solution is used only for evaluation/plotting
# ============================================================
...
class PaperConfig:
    ...
    seed: int = 1234
    device: str = "cuda:5"          # "auto", "cpu", "cuda", "cuda:0", ...
    dtype: str = "float32"        # "float32" recommended; "float64" for debugging
    output_dir: str = "runs_burgers1d_trace_ratio_paper"
    experiment_name: str = "burgers1d_riemann_trace_ratio_single_model"
    ...
    # Training budget
    # These are fixed iterations with newly resampled collocation points each step.
    ...
    # Trace-ratio gate hyperparameters
    # h(p) = h_min_factor*h0 + (h_max_factor-h_min_factor)*h0*(1-p)^2
    ...
    # Optional reference baseline. Default is False because the main method is a single model.
    # For paper comparison tables, set this to True and the code will train a vanilla continuation
    # from the same warm-up checkpoint using the same gated-phase budget.
    run_vanilla_reference: bool = True
...
def configure_runtime(new_cfg: PaperConfig):
...
def set_seed(seed: int):
    ...
    # Deterministic algorithms can slow PINN training substantially, so they are not forced here.
...
def make_run_dir(cfg: PaperConfig) -> Path:
...
def shock_speed(cfg: PaperConfig = cfg) -> float:
    if abs(cfg.uL - cfg.uR) < 1.0e-14:
        return float(cfg.uL)
    return float((0.5 * cfg.uL ** 2 - 0.5 * cfg.uR ** 2) / (cfg.uL - cfg.uR))


def x_shock_np(t, cfg: PaperConfig = cfg):
    return cfg.x0 + shock_speed(cfg) * np.asarray(t)


def exact_solution_np(X, T, cfg: PaperConfig = cfg):
    xs = x_shock_np(T, cfg)
    return np.where(X < xs, cfg.uL, cfg.uR).astype(np.float64)


def state_scale(cfg: PaperConfig = cfg) -> float:
    return max(abs(cfg.uL - cfg.uR), 1.0e-8)


def h0(cfg: PaperConfig = cfg) -> float:
    # Characteristic collocation spacing in the x direction.
    return (cfg.x_max - cfg.x_min) / math.sqrt(cfg.n_f)


def schedule_from_progress(progress: float, cfg: PaperConfig = cfg):
...
def schedule_from_iter(iteration: int, total_iters: int, cfg: PaperConfig = cfg):
    p = iteration / max(1, total_iters)
    return schedule_from_progress(p, cfg)
...
def sample_ic(n: int, cfg: PaperConfig = cfg):
    # Balanced sampling on the left and right of the Riemann discontinuity.
    nL = n // 2
    nR = n - nL
    ...
    y = torch.cat([torch.full_like(xL, cfg.uL), torch.full_like(xR, cfg.uR)], dim=0)

    idx = torch.randperm(n, device=DEVICE)
    return x[idx], t[idx], y[idx]


def sample_bc(n: int, cfg: PaperConfig = cfg):
    # Boundary states are fixed and consistent with the exact shock before boundary interaction.
    nL = n // 2
    nR = n - nL
    ...
    yL = torch.full_like(tL, cfg.uL)
    yR = torch.full_like(tR, cfg.uR)

    x = torch.cat([xL, xR], dim=0)
    t = torch.cat([tL, tR], dim=0)
    y = torch.cat([yL, yR], dim=0)

    idx = torch.randperm(n, device=DEVICE)
    return x[idx], t[idx], y[idx]


def sample_f(n: int, cfg: PaperConfig = cfg):
...
@torch.no_grad()
def trace_ratio_gate_centered(model, x, t, h_probe: float, cmin: float, cfg: PaperConfig = cfg):
    # Centered traces at scales h and 2h.
    # The gate is detached from the computational graph, so the method performs residual reweighting
    # rather than differentiating through the shock detector.
    x_m1 = x - h_probe
    x_p1 = x + h_probe
    x_m2 = x - 2.0 * h_probe
    x_p2 = x + 2.0 * h_probe
    ...
    u_m1 = model(cat_xt(x_m1, t))
    u_p1 = model(cat_xt(x_p1, t))
    u_m2 = model(cat_xt(x_m2, t))
    u_p2 = model(cat_xt(x_p2, t))
...
def vanilla_loss(model, cfg: PaperConfig = cfg):
    x_ic, t_ic, y_ic = sample_ic(cfg.n_ic, cfg)
    x_bc, t_bc, y_bc = sample_bc(cfg.n_bc, cfg)
    x_f, t_f = sample_f(cfg.n_f, cfg)

    u_ic = model(cat_xt(x_ic, t_ic))
    u_bc = model(cat_xt(x_bc, t_bc))
    _, r = burgers_residual(model, x_f, t_f)

    loss_ic = F.mse_loss(u_ic, y_ic)
    loss_bc = F.mse_loss(u_bc, y_bc)
...
def trace_ratio_gated_loss(model, iteration: int, total_iters: int, cfg: PaperConfig = cfg):
...
def train_warmup_fixed(model, cfg: PaperConfig = cfg):
    model.train()
    ...
    for it in range(1, cfg.warmup_iters + 1):
        ...
        should_log = (it == 1 or it % cfg.history_every == 0 or it == cfg.warmup_iters)
        if should_log:
            row = {
                "phase": "warmup",
                "iter": it,
...
def train_gated_fixed(model, cfg: PaperConfig = cfg):
    model.train()
    ...
    for it in range(1, cfg.gated_iters + 1):
        ...
        should_log = (it == 1 or it % cfg.history_every == 0 or it == cfg.gated_iters)
        if should_log:
            row = {
                "phase": "gated",
                "iter": it,
        ...
        if it == 1 or it % cfg.print_every == 0 or it == cfg.gated_iters:
            print(
                ...
                f"C={parts['C_mean'].item():.3f} "
                f"Jmax={parts['Jhat_max'].item():.1f} "
                f"W={parts['W_mean'].item():.3f} "
                f"h={parts['h'].item():.4f} "
                f"cmin={parts['cmin'].item():.2f}"
            )

    return model, pd.DataFrame(history)


def train_vanilla_continuation_fixed(model, cfg: PaperConfig = cfg):
    # Optional baseline: same warm-up model, same extra budget, no gate.
    model.train()
    opt = torch.optim.AdamW(model.parameters(), lr=cfg.lr_gated)
    sch = torch.optim.lr_scheduler.CosineAnnealingLR(
        opt,
    ...
    history = []
    print("\n[Optional baseline] Vanilla continuation from the same warm-up checkpoint")
    ...
    for it in range(1, cfg.gated_iters + 1):
        ...
        if it == 1 or it % cfg.print_every == 0 or it == cfg.gated_iters:
            print(
                f"[vanilla-cont] {it:6d}/{cfg.gated_iters} "
                f"loss={float(loss.detach().cpu()):.3e} "
                f"ic={parts['ic'].item():.1e} "
                f"bc={parts['bc'].item():.1e} "
                f"pde={parts['pde'].item():.1e}"
...
def predict_points(model, x_np, t_np, batch_size: int = 65536):
    ...
    for s in range(0, x_flat.size, batch_size):
        ...
        xt = torch.tensor(
            np.stack([x_flat[s:e], t_flat[s:e]], axis=1),
            device=DEVICE,
            dtype=DTYPE,
        )
        out.append(to_numpy(model(xt)).reshape(-1))
    return np.concatenate(out, axis=0).reshape(np.asarray(x_np).shape)


@torch.no_grad()
def predict_grid(model, nx=None, nt=None, cfg: PaperConfig = cfg):
    if nx is None:
        nx = cfg.eval_nx
    if nt is None:
        nt = cfg.eval_nt
    ...
    X, T = np.meshgrid(x, t)
    U = predict_points(model, X, T)
    Ue = exact_solution_np(X, T, cfg)
    return x, t, X, T, U, Ue
...
def crossing_location(x, u, level, expected=None, window=None):
    ...
    if len(candidates) == 0:
        # Fallback: closest level point. This avoids dropping failed cases silently.
...
def shock_width_from_profile(x, u, cfg: PaperConfig, t_value, high_frac=0.95, low_frac=0.05):
    hi = cfg.uR + high_frac * (cfg.uL - cfg.uR)
    lo = cfg.uR + low_frac * (cfg.uL - cfg.uR)
    expected = float(x_shock_np(t_value, cfg))
    window = max(0.15, 10.0 * (x[1] - x[0]))
    x_hi = crossing_location(x, u, hi, expected=expected, window=window)
    x_lo = crossing_location(x, u, lo, expected=expected, window=max(0.35, 2.0 * window))
    return abs(x_lo - x_hi)


def compute_error_and_shock_metrics(model, cfg: PaperConfig = cfg):
    x, t, X, T, U, Ue = predict_grid(model, cfg.eval_nx, cfg.eval_nt, cfg)
    err = U - Ue
    ...
    final_err = err[-1]
    final_exact = Ue[-1]
    final_l1 = float(np.mean(np.abs(final_err)))
    final_l2 = float(np.sqrt(np.mean(final_err ** 2)))
    final_rel_l2 = float(final_l2 / (np.sqrt(np.mean(final_exact ** 2)) + 1.0e-12))
    ...
    for i, tt in enumerate(t):
        if tt < max(0.02, 2.0 * (t[1] - t[0])):
            continue
        expected = float(x_shock_np(tt, cfg))
        row = U[i]
        x_mid = crossing_location(x, row, mid, expected=expected, window=0.35)
        pos_pred.append(x_mid)
        time_used.append(tt)
        pos_errs.append(abs(x_mid - expected))
        widths_95_05.append(shock_width_from_profile(x, row, cfg, tt, 0.95, 0.05))
        widths_90_10.append(shock_width_from_profile(x, row, cfg, tt, 0.90, 0.10))

    pos_pred = np.asarray(pos_pred)
    time_used = np.asarray(time_used)

    if len(time_used) >= 2:
        speed_pred = float(np.polyfit(time_used, pos_pred, 1)[0])
    else:
        speed_pred = np.nan

    speed_exact = shock_speed(cfg)
    final_x_pred = crossing_location(
        x, U[-1], mid,
        expected=float(x_shock_np(cfg.t_max, cfg)),
        window=0.35,
    )
    final_x_exact = float(x_shock_np(cfg.t_max, cfg))

    tv_each = np.sum(np.abs(np.diff(U, axis=1)), axis=1)
    tv_exact = state_scale(cfg)
    tv_excess = np.maximum(tv_each - tv_exact, 0.0)
    ...
    return {
        ...
        "shock_pos_final_error": float(abs(final_x_pred - final_x_exact)),
        "shock_pos_final_pred": float(final_x_pred),
        "shock_pos_final_exact": float(final_x_exact),
        ...
        "tv_mean": float(np.mean(tv_each)),
        "tv_excess_mean": float(np.mean(tv_excess)),
        "dx_eval": float(x[1] - x[0]),
    }


def compute_global_conservation_metrics(model, cfg: PaperConfig = cfg):
    x, t, X, T, U, Ue = predict_grid(model, cfg.eval_nx, cfg.eval_nt, cfg)
    mass = np.trapz(U, x, axis=1)
    mass_exact = np.trapz(Ue, x, axis=1)

    f_left = flux_np(U[:, 0])
    f_right = flux_np(U[:, -1])
    flux_diff = f_right - f_left

    cons = []
    rel = []
    for i in range(len(t)):
        integral_flux = np.trapz(flux_diff[: i + 1], t[: i + 1]) if i > 0 else 0.0
        val = mass[i] - mass[0] + integral_flux
        cons.append(val)
        denom = abs(mass[i] - mass[0]) + abs(integral_flux) + 1.0e-12
        rel.append(abs(val) / denom)
    ...
    return {
        "global_cons_mean_abs": float(np.mean(np.abs(cons))),
        "global_cons_max_abs": float(np.max(np.abs(cons))),
        "global_cons_final_abs": float(abs(cons[-1])),
        "global_cons_mean_rel": float(np.mean(rel[1:])),
        "mass_error_mean_abs": float(np.mean(np.abs(mass_err))),
        "mass_error_final_abs": float(abs(mass_err[-1])),
    }


def compute_local_conservation_metrics(model, cfg: PaperConfig = cfg):
    rng = np.random.default_rng(cfg.seed + 314159)
    vals = []
    rels = []

    for _ in range(cfg.n_control_volumes):
        # Sample a nondegenerate space-time control volume.
        ...
        u_t1 = predict_points(model, xq, np.full_like(xq, t1))
        u_t2 = predict_points(model, xq, np.full_like(xq, t2))
        mass_t1 = np.trapz(u_t1, xq)
        mass_t2 = np.trapz(u_t2, xq)

        u_a = predict_points(model, np.full_like(tq, a), tq)
        u_b = predict_points(model, np.full_like(tq, b), tq)
        flux_int = np.trapz(flux_np(u_b) - flux_np(u_a), tq)

        cv_res = mass_t2 - mass_t1 + flux_int
        denom = abs(mass_t2) + abs(mass_t1) + abs(flux_int) + 1.0e-12
        vals.append(abs(cv_res))
        rels.append(abs(cv_res) / denom)

    vals = np.asarray(vals)
    rels = np.asarray(rels)
    return {
        "local_cons_cv_mean_abs": float(np.mean(vals)),
        "local_cons_cv_median_abs": float(np.median(vals)),
        "local_cons_cv_max_abs": float(np.max(vals)),
        "local_cons_cv_mean_rel": float(np.mean(rels)),
        "local_cons_cv_max_rel": float(np.max(rels)),
    }


@torch.no_grad()
def compute_gate_diagnostics(model, progress=1.0, cfg: PaperConfig = cfg):
    ...
    if V.sum() == 0:
        # This can occur only in very small debugging runs with h larger than the domain.
        # Paper settings use sufficiently small h, but falling back to all points makes diagnostics robust.
        V = np.ones_like(V, dtype=bool)

    shock_band = np.abs(X - x_shock_np(T, cfg)) <= 2.0 * h_probe
    active = G > 0.5
    active_loose = G > 0.1
...
def evaluate_model(model, model_name: str, cfg: PaperConfig = cfg):
    row = {
        "model": model_name,
        "seed": cfg.seed,
        **compute_error_and_shock_metrics(model, cfg),
        ...
        **compute_gate_diagnostics(model, progress=1.0, cfg=cfg),
        "xshock_tmax_exact": float(x_shock_np(cfg.t_max, cfg)),
    }
    return row
...
def overlay_shock_line(ax, cfg: PaperConfig = cfg, color="w", lw=1.5):
    tt = np.linspace(cfg.t_min, cfg.t_max, 200)
    xs = x_shock_np(tt, cfg)
    ax.plot(xs, tt, "--", color=color, lw=lw, alpha=0.9)


def plot_solution_error_gate(model, cfg: PaperConfig = cfg, save_path: Path = None):
    # Plot on a smaller grid for speed/visual readability.
    x, t, X, T, U, Ue = predict_grid(model, cfg.plot_nx, cfg.plot_nt, cfg)
    err = np.abs(U - Ue)
    ...
    with torch.no_grad():
        G, C, Jhat, Jbar, valid = trace_ratio_gate_centered(
            model, xt[:, 0:1], xt[:, 1:2], h_probe, cmin, cfg
        )
    G = to_numpy(G).reshape(cfg.plot_nt, cfg.plot_nx)

    fig, axs = plt.subplots(1, 4, figsize=(16.0, 3.6), constrained_layout=True)
    panels = [
        (Ue, "Exact entropy solution", "viridis", cfg.uR, cfg.uL),
        (U, "Prediction", "viridis", cfg.uR, cfg.uL),
        (err, "Absolute error", "magma", 0.0, None),
        (G, f"Trace-ratio gate g\nh={h_probe:.4f}, cmin={cmin:.2f}", "viridis", 0.0, 1.0),
    ]

    for ax, (A, title, cmap, vmin, vmax) in zip(axs, panels):
        im = ax.imshow(
            A,
        ...
        overlay_shock_line(ax, cfg, color="w" if cmap != "magma" else "cyan")
        ax.set_title(title)
        ax.set_xlabel("x")
        ax.set_ylabel("t")
...
def plot_time_slices(model, cfg: PaperConfig = cfg, save_path: Path = None, reference_model=None):
    times = [cfg.t_min, 0.25 * cfg.t_max, 0.50 * cfg.t_max, 0.75 * cfg.t_max, cfg.t_max]
    x = np.linspace(cfg.x_min, cfg.x_max, cfg.slice_nx)

    fig, axs = plt.subplots(1, len(times), figsize=(18.0, 3.5), constrained_layout=True)

    for ax, tt in zip(axs, times):
        T = np.full_like(x, tt)
        Ue = exact_solution_np(x, T, cfg)
        Up = predict_points(model, x, T)

        ax.plot(x, Ue, "k-", lw=2.4, label="Exact")
        if reference_model is not None:
            Ur = predict_points(reference_model, x, T)
            ax.plot(x, Ur, "--", lw=1.8, label="Vanilla ref.")
        ax.plot(x, Up, "-", lw=2.0, label="Trace-ratio gated")
        ax.axvline(float(x_shock_np(tt, cfg)), color="k", ls=":", lw=1.0, alpha=0.7)
...
def plot_training_history(history_df: pd.DataFrame, cfg: PaperConfig = cfg, save_path: Path = None):
    if history_df.empty:
        return

    fig, ax = plt.subplots(figsize=(7.2, 4.0), constrained_layout=True)
    ax.semilogy(history_df["total_iter"], history_df["loss"], label="total loss")
    if "ic" in history_df:
        ax.semilogy(history_df["total_iter"], history_df["ic"], label="IC MSE", alpha=0.8)
    if "bc" in history_df:
        ax.semilogy(history_df["total_iter"], history_df["bc"], label="BC MSE", alpha=0.8)
    if "weighted_pde" in history_df:
        ax.semilogy(history_df["total_iter"], history_df["weighted_pde"], label="PDE term", alpha=0.8)
    ax.axvline(cfg.warmup_iters, ls="--", color="k", lw=1.0, alpha=0.7)
...
def plot_gate_history(history_df: pd.DataFrame, cfg: PaperConfig = cfg, save_path: Path = None):
    gated = history_df[history_df["phase"] == "gated"].copy()
    if gated.empty:
        return

    fig, ax1 = plt.subplots(figsize=(7.2, 4.0), constrained_layout=True)
    ax1.plot(gated["total_iter"], gated["gate_mean"], label="mean(g)")
    ax1.plot(gated["total_iter"], gated["gate_active_gt_0p5"], label="frac(g>0.5)")
    ax1.plot(gated["total_iter"], gated["gate_active_gt_0p1"], label="frac(g>0.1)")
    ax1.set_xlabel("Training iteration")
    ax1.set_ylabel("Gate statistic")
    ax1.grid(alpha=0.25)
    ax1.legend(loc="upper left")

    ax2 = ax1.twinx()
    ax2.plot(gated["total_iter"], gated["h"], ls="--", label="h")
    ax2.plot(gated["total_iter"], gated["cmin"], ls=":", label="cmin")
    ax2.set_ylabel("Schedule")
    ax2.legend(loc="upper right")
    ax1.set_title("Gate statistics and schedule")
...
# ============================================================
# 12. Main experiment function
# ============================================================

def print_experiment_header(cfg: PaperConfig, run_dir: Path):
    h_start, c_start, _ = schedule_from_progress(0.0, cfg)
    h_end, c_end, _ = schedule_from_progress(1.0, cfg)

    print("=" * 90)
    print("1D inviscid Burgers Riemann shock: single-model trace-ratio gated PINN")
    print("=" * 90)
    print(f"seed                 : {cfg.seed}")
    print(f"device               : {DEVICE}")
    print(f"dtype                : {cfg.dtype}")
    print(f"domain               : x in [{cfg.x_min}, {cfg.x_max}], t in [{cfg.t_min}, {cfg.t_max}]")
    print(f"initial condition    : uL={cfg.uL}, uR={cfg.uR}, x0={cfg.x0}")
    print(f"boundary condition   : u({cfg.x_min},t)={cfg.uL}, u({cfg.x_max},t)={cfg.uR}")
    print("PDE                  : u_t + (u^2/2)_x = 0")
    print(f"shock speed exact    : {shock_speed(cfg):.8f}")
    print(f"x_shock(t_max) exact : {float(x_shock_np(cfg.t_max, cfg)):.8f}")
    print(f"network              : width={cfg.width}, depth={cfg.depth}, activation={cfg.activation}")
    print(f"training budget      : warmup={cfg.warmup_iters}, gated={cfg.gated_iters}")
    ...
    print("exact solution       : used only for evaluation and plotting")
    print(f"run directory        : {run_dir}")
    print("=" * 90)


def run_paper_experiment(cfg: PaperConfig = cfg):
    configure_runtime(cfg)
    set_seed(cfg.seed)
    run_dir = make_run_dir(cfg)

    if cfg.uL <= cfg.uR:
        raise ValueError("This script is written for a shock Riemann problem with uL > uR.")

    print_experiment_header(cfg, run_dir)
    ...
    if cfg.save_outputs:
        ...
        save_json(meta, run_dir / "config_and_runtime.json")
    ...
    # Single model: warm-up -> gated phase.
    model, hist_warmup = train_warmup_fixed(model, cfg)

    warmup_state = clone_state(model)
    vanilla_reference = None
    hist_ref = pd.DataFrame()

    if cfg.run_vanilla_reference:
        # Optional baseline for paper comparison. This is not used in the method run.
        vanilla_reference = MLP(cfg).to(DEVICE, dtype=DTYPE)
        vanilla_reference.load_state_dict(warmup_state)
        vanilla_reference, hist_ref = train_vanilla_continuation_fixed(vanilla_reference, cfg)

    model, hist_gated = train_gated_fixed(model, cfg)

    elapsed = time.time() - t0
    print(f"\nTraining wall time: {elapsed:.2f} seconds")

    history = pd.concat([hist_warmup, hist_gated], ignore_index=True)
    if cfg.save_outputs:
        history.to_csv(run_dir / "history_trace_ratio_single.csv", index=False)
        if not hist_ref.empty:
            hist_ref.to_csv(run_dir / "history_vanilla_reference.csv", index=False)
        torch.save(model.state_dict(), run_dir / "checkpoint_final_trace_ratio_single.pt")
        torch.save(warmup_state, run_dir / "checkpoint_warmup_final.pt")
        if vanilla_reference is not None:
            torch.save(vanilla_reference.state_dict(), run_dir / "checkpoint_final_vanilla_reference.pt")

    print("\nEvaluating final trace-ratio gated model...")
    rows = [evaluate_model(model, "ours_trace_ratio_single_final", cfg)]

    if vanilla_reference is not None:
        print("Evaluating optional vanilla reference...")
        rows.append(evaluate_model(vanilla_reference, "vanilla_reference_final", cfg))
    ...
    # Figures
    plot_solution_error_gate(
        model,
        cfg,
        save_path=(run_dir / "fig_solution_error_gate.png") if cfg.save_outputs else None,
    )
    plot_time_slices(
        model,
        cfg,
        reference_model=vanilla_reference,
        save_path=(run_dir / "fig_time_slices.png") if cfg.save_outputs else None,
    )
    plot_training_history(
        history,
        cfg,
        save_path=(run_dir / "fig_training_history.png") if cfg.save_outputs else None,
    )
    plot_gate_history(
        history,
        cfg,
        save_path=(run_dir / "fig_gate_history.png") if cfg.save_outputs else None,
    )
...
# For a paper main result, run this cell as is.
# If GPU memory is tight, reduce cfg.eval_nx/eval_nt for debugging first, not for final reporting.

#results = run_paper_experiment(cfg)
...
def gpinn_residual_scaled_components(model, coordinates, cfg):
    ...
    flux_x = torch.autograd.grad(
        flux, x, torch.ones_like(flux), create_graph=True, retain_graph=True
    )[0]
    residual = (u_t + flux_x) / state_scale(cfg)
    return (residual,), (x, t)
...
GPINN_HELPER_RECONSTRUCTION_SHA256 = 'ac9cb7f2041baeb1c302c2b4351d720c29bbe2487262c9cc02ab370e6eb87c41'
